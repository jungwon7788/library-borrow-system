...
def render_member():
    ...
    # ===== RESET FLAG (ต้องอยู่ก่อนสร้าง widget) =====
    ...
    with st.form("member_form"):
        ...
        with col_a:
            member_code = st.text_input(
                "รหัสสมาชิก (เช่น M001)", max_chars=10, key="member_code"
            )
            member_name = st.text_input("ชื่อ - สกุล", key="member_name")
            gender = st.selectbox(
                "เพศ", ["ไม่ระบุ", "หญิง", "ชาย", "อื่น ๆ"], key="gender"
            )
        ...
        with col_b:
            ...
            is_active = st.checkbox(
                "ยังใช้งานอยู่", value=True, key="is_active"
            )
    ...
    if submitted:
        ok, msgs = controller.create_member(
            member_code, member_name, gender, email, phone, is_active
        )
        if not ok:
            for m in msgs:
                st.error("⚠ " + m)
        ...
        for _, row in members_df.iterrows():
            ...
            with c4:
                if st.button("ลบ", key=f"delete_member_{row['id']}"):
                    ...
                    st.success(
                        f"ลบสมาชิก {row['ชื่อสกุล']} เรียบร้อยแล้ว"
                    )
                    st.rerun()
    ...
    selected = st.selectbox(
        "เลือกสมาชิกที่จะแก้ไข", member_options, key="selected_member"
    )
    ...
    with st.form("edit_member_form"):
        ...
        with col1:
            edit_member_code = st.text_input(
                "รหัสสมาชิก", value=selected_row["รหัสสมาชิก"]
            )
            edit_name = st.text_input(
                "ชื่อ - สกุล", value=selected_row["ชื่อสกุล"]
            )
        ...
        with col2:
            edit_email = st.text_input(
                "อีเมล", value=selected_row["อีเมล"] or ""
            )
            edit_phone = st.text_input(
                "เบอร์โทรศัพท์", value=selected_row["เบอร์โทร"] or ""
            )
